...
/**
 * @brief 在缓存中查找请求涉及的所有cache line的映射关系
 *
 * @param req OCF请求结构体，包含了需要查找的core line范围信息
 *
 * 该函数遍历请求中的所有core line，检查它们在cache中的状态（命中/未命中）
 */
static void ocf_engine_lookup(struct ocf_request* req) {
    ...
    OCF_DEBUG_PARAM(cache, "Sequential - %s", ocf_engine_is_sequential(req) ? "Yes" : "No");
}
