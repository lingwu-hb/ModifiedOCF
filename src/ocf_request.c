...
struct ocf_request* ocf_req_new(ocf_queue_t queue, ocf_core_t core, uint64_t addr, uint32_t bytes, int rw) {
...
int ocf_req_alloc_map(struct ocf_request* req) {
    ...
    req->map = env_zalloc(ocf_req_sizeof_map(req) +
                              ocf_req_sizeof_alock_status(req),
                          ENV_MEM_NOIO);
    if (!req->map) {
...
struct ocf_request* ocf_req_new_extended(ocf_queue_t queue, ocf_core_t core, uint64_t addr, uint32_t bytes, int rw) {
    struct ocf_request* req;
...
struct ocf_request* ocf_req_new_discard(ocf_queue_t queue, ocf_core_t core, uint64_t addr, uint32_t bytes, int rw) {
    struct ocf_request* req;
