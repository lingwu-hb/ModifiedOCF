...
err:
    ocf_cache_log(cache, log_err,
                  "Core with given uuid not found "
                  "in cache metadata\n");
...
void ocf_mngt_cache_add_core(ocf_cache_t cache,
    ...
    result = ocf_pipeline_create(&pipeline, cache, cfg->try_add ? &ocf_mngt_cache_try_add_core_pipeline_props : &ocf_mngt_cache_add_core_pipeline_props);
    if (result)
...
static void _ocf_mngt_cache_remove_core(ocf_pipeline_t pipeline, void* priv, ocf_pipeline_arg_t arg) {
    struct ocf_mngt_cache_remove_core_context* context = priv;
    ocf_cache_t cache = context->cache;
    ocf_core_t core = context->core;
...
static int _cache_mngt_set_core_seq_cutoff_promo_count(ocf_core_t core,
    ...
    ocf_core_log(core, log_info,
                 "Changing sequential cutoff promotion "
