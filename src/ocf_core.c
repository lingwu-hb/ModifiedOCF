...
ocf_core_state_t ocf_core_get_state(ocf_core_t core) {
    ...
    return core->opened ? ocf_core_state_active : ocf_core_state_inactive;
}
...
static int ocf_core_submit_io_fast(struct ocf_io* io, struct ocf_request* req, ocf_core_t core, ocf_cache_t cache) {
...
#if OCF_DEBUG_ENABLED
        printf("[Debug] IO Hit Address: %llu, Size: %u\n", req->ioi.io.addr, req->ioi.io.bytes);
#endif
...
#if OCF_DEBUG_ENABLED
    printf("[Debug] IO MISS Address: %llu, Size: %u\n", req->ioi.io.addr, req->ioi.io.bytes);
#endif
